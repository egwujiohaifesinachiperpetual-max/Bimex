...
import { walletKit } from "../stellar/walletKitInstance.js";
import { CONFIG } from "../stellar/contrato.js";
...
export default function ConectarWallet({ onConectado, autoConectar = true, inNavbar = false }) {
  ...
  // Try to restore previous wallet on mount
  useEffect(() => {
    if (!autoConectar) return;
    (async () => {
      try {
        // If a wallet was previously selected, it will be restored automatically by walletKitInstance
        const wallet = walletKit.getCurrentWallet();
        if (wallet) {
  ...
  async function conectar() {
    ...
    setError("");
    try {
      // Open the built‑in modal (or switch wallet if none selected)
      await walletKit.selectWallet();
      const address = await walletKit.getAddress();
      // Verify we are on the correct network (optional – kit will error otherwise)
      const networkPassphrase = await walletKit.getNetworkPassphrase?.();
      if (networkPassphrase && networkPassphrase !== CONFIG.NETWORK_PASSPHRASE) {
        setEstado("red_incorrecta");
        return;
      }
      if (!address) {
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12 }}>
      ...
      {estado === "sin_extension"" && (
        <p style={{ color: "var(--amber)", fontSize: "0.82rem", margin: 0 }}>
          No se detectó ninguna extensión de wallet. Instala Freighter o usa WalletConnect.
        </p>
      )}

      {estado === "red_incorrecta" && (
        <p style={{ color: "var(--amber)", fontSize: "0.82rem", margin: 0 }}>
          Cambia la red de la wallet a <strong>Testnet</strong>.
        </p>
      )}
